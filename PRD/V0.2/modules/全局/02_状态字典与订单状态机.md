...
> 全局 PRD 草案。
> 目标:统一三类长租订单、短租订单、账单、支付、合同、公证、发货、设备、押金、控车、续租、租后、客诉等状态,避免不同页面各自定义状态导致数据不互通。
...
> - 新增短租订单主状态机(§3.2 + §5.2)
> - 新增 §6.8 押金状态、§6.9 控车日志状态、§6.10 续租链状态、§6.11 骑行人认证状态
> - §6.6 设备状态明确"长短租共用"
...
3. **短租订单与长租订单状态机独立,不能混用枚举。**通过 `business_line` 字段区分(assurance_rent / experience_rent)。
...
| PENDING_FUNDER | 待分配资方 | 分红/平台订单未分配资方(运营端内部状态,**不暴露 C 端**) |
...
**C 端口径**:`PENDING_FUNDER` 和 `PENDING_STORE_ASSIGN` 都是后台内部状态,客户侧统一显示"平台审核中",**不暴露具体子状态**。

### 3.2 短租订单主状态(business_line = experience_rent)⭐ 新增

| 状态编码 | 状态名称 | 说明 |
|---|---|---|
| SR_PENDING_PAY | 待支付租金 | 订单已创建,客户待支付租金 |
| SR_PENDING_DEPOSIT | 押金待支付/待授权 | 租金已付,押金待支付(实付场景)或待授权(免押/预授权场景) |
| SR_PENDING_CONFIRM | 待商家接单 | 押金完成,商家 SLA 时钟启动 |
...
| 分账目标 | 门店钱包(**仅后台**) | 门店/资方按比例(**仅后台**) | **分配的履约门店 + 平台(仅后台)** |
| 渠道 | 默认不统计 | 可统计 | 可统计 |
| 履约动作权限 | 门店全权 | 门店全权 | **门店仅发货/售后,无审核/改单权** |

**C 端口径**(2026-05-25 决策):客户感知统一为"平台审核",订单类型(门店/分红/平台)不暴露 C 端;客户合同签署后**只看到"提货门店:XXX"一个字段**。
...
**流转关键说明**:
- 不是每个订单都经过所有状态。
- **门店订单**:跳过 `PENDING_FUNDER` 和 `PENDING_STORE_ASSIGN`,直接 `PENDING_REVIEW → PENDING_SIGN`(若有合同)或 `→ PENDING_SHIP`。
- **分红订单**:`PENDING_REVIEW → PENDING_FUNDER → PENDING_SIGN`(门店已知,不分配)。
- **平台订单**:`PENDING_REVIEW → PENDING_FUNDER → PENDING_STORE_ASSIGN → PENDING_SIGN`,或不需资方时 `PENDING_REVIEW → PENDING_STORE_ASSIGN → PENDING_SIGN`。
...
```mermaid
flowchart TD
    A["SR_PENDING_PAY 待支付租金"] --> B["SR_PENDING_DEPOSIT 押金待支付/授权"]
    ...
    I -->|客户处理后还车| G
    F --> J["[链中下一单]"]
    G -->|验收完成 押金处理| K["SR_COMPLETED 已完成"]
...
**路径分叉说明**:
- 信用免押场景:`SR_PENDING_PAY → SR_PENDING_CONFIRM`(跳过押金待支付,因为租金+免押授权可一次完成)
- 实付押金 / 预授权场景:`SR_PENDING_PAY → SR_PENDING_DEPOSIT → SR_PENDING_CONFIRM`
...
**长短租共用规则**:
- 设备物理上属于同一张 `devices` 主表
- 短租车辆通过 `short_rent_vehicle_ext` 扩展表关联(详见短租 05 §4.3)
- 状态机统一在本表,业务方按 `business_line` 解释语义
- 同一设备同一时刻只能服务一个业务线一个订单
...
| 编码 | 名称 | 说明 |
|---|---|---|
| DEPOSIT_PENDING | 待支付/待授权 | 订单已创建,押金未完成 |
...
| CHAIN_CLOSED | 已闭链 | 链终结(客户完整还车,押金结算完成) |
| CHAIN_DISPUTED | 争议中 | 链中订单存在押金 / 损坏争议 |
...
| 商家操作 | 审核门店订单、发货、上传材料、归还验收 | 接单、核销、还车验收、押金扣款 |
| 运营操作 | 审核、分配资方、**分配履约门店**(平台订单)、发起合同、退款、关闭 | 代接单、代核销、押金大额审批、违约金免除 |
...
| 字段 | 说明 |
|---|---|
| 对象类型 | 订单、账单、合同、设备、押金、控车、续租链、骑行人、**门店分配**、投诉等 |
| 对象 ID | 业务 ID |
...
| 2026-05-26 | §3.2 §5.2 §6.8-6.11 §9 v1.1 | **短租补全**:1. §3.2 新增短租主状态机(SR_* 前缀);2. §5.2 新增短租状态流转图;3. §6.6 设备状态明确长短租共用;4. §6.8 押金状态;5. §6.9 控车日志状态;6. §6.10 续租链状态;7. §6.11 骑行人认证状态;8. §9 业务线编码统一为 assurance_rent / experience_rent |
