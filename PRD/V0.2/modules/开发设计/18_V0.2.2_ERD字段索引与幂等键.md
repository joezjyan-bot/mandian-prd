...
erDiagram
    ...
    orders ||--|| device_asset : assetizes
    device_asset ||--o{ device_asset_status_log : transitions
    orders ||--o{ device_value_confirmation : confirms
```
...
### 3.9 设备资产记录 `device_asset`

| 字段 | 说明 |
|---|---|
| `asset_id` | 主键 |
| `order_id` | 安心用长租订单,订单生成后自动资产化 |
| `merchant_id` / `store_id` | 归属商家/门店 |
| `product_id` / `sku_id` | 商品与规格 |
| `brand_model` | 品牌型号 |
| `condition_label` | 成色,如全新/95新/9成新等 |
| `device_value_cent` | 设备价值,等于该订单设备价格 |
| `source_type` | merchant_self/platform_supply/other,内部字段,C端禁返 |
| `device_identifier` | IMEI/SN/VIN;待实物绑定阶段可空,交付时回填 |
| `identifier_type` | IMEI/SN/VIN/OTHER |
| `ownership_status` | platform/merchant/customer/funder_internal,内部字段,C端禁返 |
| `delivery_evidence_ids` | 交付照片/凭证附件 ID 集合 |
| `asset_status` | PENDING_ASSETIZED/DELIVERED/IN_LEASE/RETURNED/PURCHASED/DISPOSED |
| `created_at` / `updated_at` | 创建/更新时间 |

约束:

- 安心用长租一单一资产;未来一单多设备时扩展为订单对资产一对多。
- `device_identifier` 在 `PENDING_ASSETIZED` 阶段允许为空,交付回填后必须进入一致性校验。
- C 端只展示设备型号、成色、设备价值等确认信息,不得返回来源类型、内部权属、内部资金来源等字段。

### 3.10 设备资产状态日志 `device_asset_status_log`

| 字段 | 说明 |
|---|---|
| `log_id` | 主键 |
| `asset_id` | 设备资产 |
| `before_status` / `after_status` | 状态流转前后值 |
| `trigger_source` | order_event/manual/callback |
| `operator_id` | 操作人或系统任务 |
| `reason` | 流转原因 |
| `created_at` | 记录时间 |

约束:

- 资产状态不得只覆盖主表,每次资产化、交付、出租、归还、购买、处置都必须写状态日志。
- 资产处置、价值修改、识别码绑定属于审计动作,须关联操作日志或审计日志。

### 3.11 设备价值确认 `device_value_confirmation`
...
| `order_id` | 订单 |
| `asset_id` | 设备资产,交付后绑定 |
| `brand_model` / `spec` / `condition_label` | 客户确认的型号、配置、成色 |
| `device_identifier` | SN/IMEI/VIN;确认时可空,交付回填 |
| `store_price_cent` | 设备价值,等于该订单设备价格 |
| `platform_ref_price_cent` | 平台参考价,预留 |
| `third_party_valuation_cent` | 第三方估值价,预留,V1 可空 |
| `confirmed_value_cent` | 客户确认价,只读方案下等于 `store_price_cent` |
| `customer_confirmed` | 客户是否已勾选确认 |
| `confirmed_at` | 确认时间 |
| `confirm_ip` | 确认 IP |
| `confirm_device_info` | 确认设备信息 |
| `page_version` | 订单确认页版本 |
| `snapshot_id` / `snapshot_hash` | 页面快照与防篡改 hash |
| `consistency_status` | consistent/reconfirm_required |
| `created_at` / `updated_at` | 创建/更新时间 |

约束:

- 设备价值确认记录作为合同附件和法诉证据包来源,不得被覆盖式修改。
- 交付实物型号/成色与确认记录不一致时,`consistency_status` 必须进入 `reconfirm_required`,并触发客户重新确认。
- 客户侧展示为“设备价值确认”,不得出现资方、资金来源、风控、平台利润等内部字段。
...
订单确认、设备价值确认、到期选择、提前归还结算等会影响客户权利义务的页面,必须保存页面快照。快照用于合同附件、争议处理、客服核查和法诉证据包,不得只保存“客户已勾选”布尔值。
...
|---|---|
| `snapshot_id` | 快照主键 |
| `snapshot_type` | order_confirm/device_value_confirmation/lease_end_choice/early_return_settlement 等 |
| `order_no` / `order_id` | 订单号和订单 ID |
| `customer_id` | 客户 ID |
| `captured_at` | 快照生成时间戳 |
| `client_ip` | 客户操作 IP |
| `client_device_info` | 客户设备信息、User-Agent、端类型 |
| `device_info_snapshot` | 设备型号、规格、成色、设备识别码、设备价值等页面展示信息 |
| `contract_version` | 合同模板版本或协议版本 |
| `page_version` | 页面版本号 |
| `operator_id` | 操作人;客户自操作时为空或记录 customer 侧主体 |
| `content_hash` | 页面内容 hash,用于防篡改 |
| `snapshot_payload` | 页面展示字段 JSON,含按钮、勾选项、金额明细、提示语 |
| `related_record_id` | 关联业务记录,如 `device_value_confirmation.confirmation_id`、`lease_end_choice_log.choice_log_id`、`early_return_request.request_id` |

### 6.2 覆盖范围

| 页面/动作 | 必须快照内容 | 关联表 |
|---|---|---|
| 设备价值确认页 | 设备型号、配置、成色、设备价值、客户独立勾选项、页面版本 | `device_value_confirmation` |
| 到期三选一页 | 归还设备 / 继续使用 / 申请购买三选项、客户选择、展示时间 | `lease_end_choice_log` |
| 提前归还预结算页 | 已使用时间、已发生费用、预计检测费、物流费、折旧补偿、保证金预计退还、客户确认 | `early_return_request` |

约束:

- 快照记录不得覆盖修改;页面重开、金额变化、客户重新确认时生成新快照。
- 法诉证据包引用快照时必须同时引用 `snapshot_id` 和 `content_hash`。
- C 端接口只返回客户可见字段,不得返回内部审核备注、资金来源、合作模式、平台利润、商家结算价等敏感字段。
