...
> 解决 P0 问题 Q5:续租生成新订单,但**父子订单状态联动 / 押金跨订单复用 / 财务对账归属**全部没有数据建模。
> 本文统一定义续租链模型、父子订单状态联动、押金链、财务归属规则。
...
| **deposit_chain_id** | bigint | 押金链 ID(关联 short_rent_deposit) |
...
`short_rent_deposit.order_id` 改为引用**首租订单**;新增 `deposit_chain_id` 作为押金标识。续租时复用同一笔押金,不创建新押金记录。
...
| 规则 | 说明 |
|---|---|
| **同链共用押金** | 续租不重复收押金,直接复用首租押金记录 |
| **押金随链结算** | 链终结(完整还车)时一次性结算押金 |
| **押金可在链中扣** | 续租期间发生损坏,可在押金上扣款,但不影响链继续 |
| **押金不足时** | 提示客户补缴 / 暂停续租能力 |
...
├── 续租 1 order_id=102 (chain_sequence=2, deposit_chain_id=1) ← 复用押金
├── 续租 2 order_id=103 (chain_sequence=3, deposit_chain_id=1) ← 复用押金
...
| 类型 | 到期 | 处理 |
|---|---|---|
| 实付押金 | 无期限(账户保留) | 链终结时统一退款 |
| 信用免押 | 通常 30-90 天 | 到期前 7 天系统提醒;到期当天**禁止再续租**;客户需还车或转实付押金 |
...
| 押金状态 = paid / authorized | "押金状态异常,请联系客服" |
| 押金扣款累计 < 押金额的 80% | "押金余额不足,请先处理待扣款" |
| 押金类型有效期未到 | "信用免押即将到期,请还车或更换支付方式" |
...
| 押金扣款(发生在续租 2)| 该笔扣款记录 deduction_id 关联**实际发生时的订单** | chain_id |
| 押金退款(链终结)| 关联**首租订单** + chain_id | chain_id |
...
还车完成 → 押金最终结算
  ↓
押金应退 = amount_paid + amount_authorized
...
| 日期 | 版本 | 修订 |
|---|---|---|
| 2026-05-25 | v1.0 | 初版,解决 P0 Q5(续租链/父子订单状态/押金链/财务归属) |
