...
| 分账 | 抽佣后入门店钱包(**仅后台**) | 门店/资方按比例(**仅后台**) | 资方/平台主收益(**仅后台**) |
| 渠道 | 默认不统计 | 可统计 | 可统计 |

**C 端口径**(2026-05-25 决策):客户感知统一为"平台审核",订单类型(门店/分红/平台)不暴露 C 端。
...
| 运营操作 | 审核、分配资方、发起合同、退款、关闭 | 代接单、代核销、押金大额审批、违约金免除 |
| 系统回调 | 支付成功、合同签署、公证完成、物流签收 | 支付成功、免押授权、中控台控车回调 |
| 定时任务 | 锁定超时释放、账单逾期、合同过期 | 接单 SLA 超时、租期到期、逾期升级、24h 锁车、预授权到期 |
...
| 字段 | 说明 |
|---|---|
| 对象类型 | 订单、账单、合同、设备、押金、控车、续租链、骑行人、投诉等 |
| 对象 ID | 业务 ID |
