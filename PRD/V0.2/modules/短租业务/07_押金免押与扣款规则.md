...
> 解决 P0 问题 Q4(押金 / 预授权 字段含义重叠)+ Q12(押金扣款审批分级)。
> 押金是体验租**最容易产生客诉**的环节,本文统一定义 4 种押金场景的数据流、扣款分级审批、退款时效。
...
| 场景 | 触发条件 | 数据特征 | 资金动作 |
|---|---|---|---|
| **A 全额实付押金** | 客户信用未达免押门槛 + 选择"实付押金" | 实际扣款到平台押金账户 | 扣款 → 还车后退款(原路退) |
...
| deposit_chain_id | bigint | 是 | 押金链 ID(支持续租复用,详见 `09_续租链与父子订单.md`) |
| order_id | bigint | 是 | 关联订单 |
| **deposit_type** | enum | 是 | `paid_full` / `credit_free` / `preauth` / `mixed`(V2 预留) |
| amount_required | decimal | 是 | 应缴押金额(标准值,从价格方案带入) |
| amount_paid | decimal | 是 | 实付金额(场景 A 为押金额,B/C 为 0) |
...
| amount_required | 应付押金 | 应付押金 | 应付押金 |
...
| 客户支付租金后 | 触发押金动作(支付 / 授权 / 验证免押) |
| 押金动作完成 | order.status 从 `pending_deposit` → `pending_confirm` |
| 商家接单 | 不触发押金动作(已完成) |
...
**客户选择权**:
- 即使满足免押条件,客户也可选择"实付押金"(场景 A)— 出于隐私或习惯
...
|---|---|---|---|---|
| **小额** | ≤ 100 元 | 门店自助 + 二次确认 | 短信 + IM 推送(扣后告知) | 门店店长 / 商家管理员 |
| **中额** | 100 - 1000 元 | 门店发起 → 平台审核 | 提前在 IM 通知客户(扣前告知)→ 平台审核 → 实扣 | 门店发起,平台审核员审批 |
| **大额** | > 1000 元 | 门店发起 → 平台审核 → 客户协商 | IM 通知客户协商 → 客户确认 / 客户申诉 → 平台终裁 | 门店发起,平台主管审批 |
...
- 单笔扣款金额 ≤ `amount_required - amount_deducted`(押金余额)
- 押金不足以覆盖扣款时,记录 `unpaid_balance`,通过线下补收 / 二次代扣处理(V2 接线上补扣)
...
| 平台自营(platform_self_operated) | 平台押金户 | 全部归平台 |
| 平台货源-门店履约(platform_asset_store_fulfillment) | 平台押金户 | 损坏赔付给平台(资产方),清洁费给门店(履约方) |

**关键**:押金账户不是商家可随意提现的钱包,是平台监管的保证金账户。
...
- `02_C端体验租下单流程.md` §7 支付和押金
- `03_商家端体验租订单与车辆.md` §7 押金扣款和退款
- `04_运营端体验租管理.md` §7 押金和退款
- `05_体验租数据模型与状态机.md` §4.5(本文为修正版)
- `09_续租链与父子订单.md`(押金跨订单复用)
...
| 日期 | 版本 | 修订 |
|---|---|---|
| 2026-05-25 | v1.0 | 初版,解决 P0 Q4(押金字段重叠)+ Q12(扣款审批分级) |
