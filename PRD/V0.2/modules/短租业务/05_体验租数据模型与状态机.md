...
stateDiagram-v2
    ...
    pending_pay --> pending_confirm: 信用免押直接通过(跳过押金支付步)
    pending_deposit --> pending_confirm: 押金支付或免押授权成功
...
| cycle_type | 租赁周期 |
| rent_price | 租金 |
| deposit_amount | 押金标准额(对应 `short_rent_deposit.amount_required`) |
...
| status | 短租订单状态(§2)|
| rent_amount | 租金 |
| deposit_amount | 押金额(冗余,实际押金状态见 short_rent_deposit) |
| paid_amount | 已付金额 |
| refund_amount | 已退金额 |
...
**v1.1 修订**:用 `07_押金免押与扣款规则.md §2.1` 的修正版结构,4 种押金场景字段不重叠。

| 字段 | 说明 |
|---|---|
| deposit_id | 押金记录 |
| deposit_chain_id | 押金链 ID(同条续租链复用,详见 `09_续租链与父子订单.md`)|
| order_id | 关联订单(首租订单)|
| deposit_type | paid_full / credit_free / preauth / mixed(V2) |
| amount_required | 应缴押金额 |
| amount_paid | 实付金额 |
...
详细字段定义、状态流、4 种场景取值对照见 `07_押金免押与扣款规则.md §2`。

### 4.6 `short_rent_fund_entries`

记录租金、押金、扣款、退款、平台抽成、门店/商家收入、履约费等资金动作。
...
| `short_rent.deposit.deduct` | 押金扣款(分级见 07 文档) |
...
| **`short_rent.overdue.exempt`** ⭐ | 违约金免除 |
...
- `07_押金免押与扣款规则.md` — 押金 4 场景与扣款分级(本文 §4.5)
- `08_控车日志与异常处置.md` — 控车日志表(本文 §4.7)
- `09_续租链与父子订单.md` — 续租链与父子状态(本文 §4.9)
- `10_体验租逾期与SLA规则.md` — 逾期违约金与接单 SLA(本文 §4.8)
