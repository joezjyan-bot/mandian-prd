...
> 来源：旧后台 `运营管理平台 / 低费率订单管理 / 低费率订单` 实测梳理。新系统统一改名为 `平台订单`：门店发起，平台/资方全额出资，运营平台负责审核、分配资方和财务主控。审核类按钮只记录入口，不执行确认。
...
旧名称：低费率订单
新名称：平台订单
发起端：门店手机端，可在商家 PC 端查看进度
审核端：运营平台
资方：运营平台分配，资方全额出资
发货：默认门店发货，保留线上物流/平台发货扩展
客服：待审核列表支持联系客服，推送订单信息并调起 IM
```
...
- 菜单路径：新系统为 `平台订单管理 / 平台订单`；旧系统为 `低费率订单管理 / 低费率订单`
...
- 入口：列表行 `查看`
- 路由：旧系统为 `/LowRateOrder/list/details?id={低费率订单ID}`；新系统路由需改为平台订单语义
- 页面标题：`详情`
- 详情标题：新系统改为 `平台订单详情`
...
客服改价、改套餐、审核结论必须写操作日志，并保存改价前后快照。
...
```mermaid
flowchart LR
  A["平台订单列表"] -->|"点击状态下拉"| B["待审核/已通过/已取消"]
  B -->|"选已通过 + 查询"| C["列表筛选为已通过记录"]
  C -->|"重置"| A
  A -->|"点击查看"| D["平台订单详情"]
  D -->|"待审核详情点击审核通过"| E["高风险：绑定方案并通过订单"]
  D -->|"待审核详情点击审核拒绝"| F["高风险：拒绝平台订单，需原因"]
```
...
| 优先级 | 问题 | 影响 | 建议 |
|---|---|---|---|
| P0 | 待审核详情的审核通过会影响平台订单和分佣快照 | 误触会改变订单资金和佣金规则 | 必须增加确认弹窗、权限校验、审计日志 |
