...
> - 新增订单关键字段快照、内部资金来源放款状态、退款工单状态、结算流水 `entry_type` 10 类枚举
> - `PENDING_FUNDER_ASSIGN` 作为待分配内部资金来源标准状态;历史 `PENDING_FUNDER` 仅作兼容别名记录,新文档优先使用标准状态
...
| 履约中 | `IN_FULFILLMENT` | 起租、账单计划生效 | 系统 | 起租时间、账单计划版本 |
| 正常结清 | `NORMAL_SETTLED` | 全部账单完成且无争议 | 系统/财务 | 结清方式、尾款、证据归档 |
| 提前留购/提前结清 | `EARLY_RETAINED` | 客户支付剩余款项,按模板完成留购或提前结清 | 客户/财务/系统 | 业务模式模板、金额计算、支付记录 |
...
说明:历史状态 `PENDING_PAYMENT`、`PENDING_SHIP`、`PENDING_RECEIVE`、`PURCHASE_PENDING`、`RENTING`、`COMPLETED`、`RETAINED` 等仅作为兼容编码或旧数据识别字段保留。新开发、页面文案、测试用例和 OpenAPI 当前契约必须使用上表主状态码,其中 `RENTING` 对应 `IN_FULFILLMENT`,`COMPLETED` 对应 `NORMAL_SETTLED`,`RETAINED` 对应 `EARLY_RETAINED`。C 端不得展示“内部资金来源分配、合作模式、风控结论”等内部状态。
...
### 0.6 IM 会话状态(session_status)

| 状态 | 含义 | V1 规则 |
|---|---|---|
| `queued` | 排队中 | 客户发起会话但未分配客服 |
| `assigned` | 已派单待接 | 系统轮询派给客服,等待接单 |
| `active` | 处理中 | 已被客服接单并单人锁定 |
| `waiting_customer` | 待客户回复 | 客服已发问,等待客户继续回复 |
| `waiting_internal` | 待内部处理 | 已转主管或业务模块处理 |
| `ticketed` | 已标记待工单 | V1 仅作为待内部跟进占位标记,不驱动完整工单流转;完整工单系统 V2 实现 |
| `closed` | 已结束 | 客服关闭或超时关闭 |

说明:V1 不新增工单接口、工单状态机或工单表。`ticketed` 仅用于会话侧标记和法诉证据归档,实际处理通过 `POST /im/sessions/{session_id}/transfer` 转主管完成;V2 再接入完整工单责任方、证据和 SLA 流程。
...
10. C 端不展示合作模式、资金来源、内部资金来源主体名称、风控结论;合同模板和按钮话术由后台 `funding_source / terminology_template_id` 决定。
...
| **PENDING_FUNDER_ASSIGN** ⭐ | **待分配内部资金来源** | 联营/平台订单未分配内部资金来源(运营端内部状态,**不暴露 C 端**);Stage 6 标准状态 |
| PENDING_FUNDER | 待分配内部资金来源 | 历史兼容别名,仅用于旧文档/旧数据迁移识别;新流程统一写 `PENDING_FUNDER_ASSIGN` |
...
### 3.2.1 短租异常工单子状态(short_rent_exception_ticket.process_status)

> 本表不是订单主状态。短租订单出现超时、损坏、事故、违章、丢车、控车失败、取车失败、还车争议时,订单主状态统一进入 `SR_DISPUTE` 或保持原主状态并关联异常工单;具体处理进度写入 `short_rent_exception_ticket.process_status`。

| process_status | 中文名 | 适用场景 | 与订单主状态关系 |
|---|---|---|---|
| `overtime_processing` | 超时处理中 | 超时未还、续租未完成、补收超时费 | 通常关联 `SR_OVERDUE` / `SR_DISPUTE` |
| `damage_processing` | 损坏处理中 | 普通损坏、维修报价、保证金扣款审批 | 通常关联 `SR_DISPUTE` |
| `accident_processing` | 事故处理中 | 严重事故、保险、交警/第三方材料 | 通常关联 `SR_DISPUTE` |
| `violation_processing` | 违章处理中 | 违章通知、骑行人匹配、追缴/申诉 | 可关联 `SR_IN_USE` / `SR_DISPUTE` |
| `lost_processing` | 丢车处理中 | 丢车、盗抢、报警、锁车定位 | 通常关联 `SR_LOCKED_DOWN` / `SR_DISPUTE` |
| `control_exception` | 控车异常 | 中控台调用失败、回调异常、车辆状态不一致 | 不得直接归责客户,需人工复核 |
| `pickup_exception` | 取车异常 | 核销失败、车辆不一致、门店未备车 | 可关联 `SR_PENDING_PICKUP` / `SR_CANCELLED` |
| `return_dispute` | 还车争议 | 客户与商家对验收结果不一致 | 通常关联 `SR_DISPUTE` |
...
| 资方 | 不需要 | 需要分配内部资金来源(后台) | 需要分配内部资金来源(后台) |
...
以下字段用于支撑三种合作模式、内部业务话术模板、合同模板、监管锁和资金放款链路。字段名为开发约束,不因 UI 话术变化而更名。
...
| `risk_status` | 风控状态快照;具体枚举以风控/内部资金来源分配文档为准 |
| `disbursement_status` | 内部资金来源放款状态快照:未触发 / 条件检查中 / 放款中 / 成功 / 失败 / 已撤销 |

### 6.14 `settlement_entry.entry_type` 枚举(10 类)
...
| `offline_compensation` | 门店线下补偿入账 | 入账 |
| `withdrawal` | 提现 | 出账 |
...
| 编码 | 名称 | 说明 |
|---|---|---|
| `pending` | 放款中 | 已触发内部资金来源放款或平台自营出款流程,等待结果 |
| `success` | 放款成功 | 内部资金来源放款或平台自营出款成功,可进入结算穿透 |
...
| 运营操作 | 审核、分配内部资金来源、**分配履约门店**(平台订单)、发起合同、退款、关闭 | 代接单、代核销、保证金大额审批、逾期费用减免 |
...
| 2026-05-27 | v1.2 | **Stage 6 同步修订**:1. 三种合作模式展示名统一为商家/联营/平台订单;2. `PENDING_FUNDER_ASSIGN` 作为待分配内部资金来源标准状态,保留 `PENDING_FUNDER` 兼容别名;3. C 端状态统一履约中 / 已完成,并补后台状态到 C 端状态映射;4. 新增订单关键字段快照;5. 新增 `entry_type` 10 类、`funder_disbursement_log.status`、`refund_workflow.status` 枚举 |

---

## 11. 已收敛执行口径

以下事项已纳入 V0.2.2 默认执行口径,不再作为“待确认”状态保留:

1. 商家订单不新增独立“商家审核中”主状态;统一使用 `PENDING_REVIEW`,并通过审核队列、审核主体和 `review_sub_status` 区分商家自主审核、平台审核和复审。
2. 长租到期提醒不进入独立主状态;默认按 `T-7 / T-3 / T-1 / 到期日` 触发提醒任务,具体提醒模板和触达方式走配置。短租默认到期前 30 分钟通过 SLA 推送提醒。
3. 争议中不冻结全部财务动作;按争议类型冻结对应动作。涉及退款、结算、保证金扣款、提现或返点的争议,默认冻结对应资金动作并写入审计日志;不影响只读查询、客服记录和法诉证据沉淀。
4. 短租 `SR_LOCKED_DOWN` 不再拆成多个主状态;车辆已锁、锁车命令发送中、车辆未响应等细节进入 `lock_command_status`、中控台回调和控车日志,避免短租主状态膨胀。
5. 平台订单履约门店分配超时不新增主状态;默认 24 小时未完成分配触发预警,高价值订单可在配置中追加更短预警阈值,但订单主状态仍保持在 `PENDING_REVIEW` 的内部子状态中。
